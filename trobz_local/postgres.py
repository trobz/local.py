...
def check_user_exists(username: str, system: str) -> bool:
    ...
    try:
        ...
        cmd = [*base_cmd, "-v", f"username={username}", "-tAc", query]
        result = subprocess.run(  # noqa: S603
...
def create_user(username: str, password: str, system: str) -> tuple[bool, str]:
    ...
    try:
        ...
        cmd = [*base_cmd, "-v", f"username={username}", "-v", f"password={password}", "-c", sql]
        result = subprocess.run(  # noqa: S603
...
def verify_connection(host: str, user: str, password: str) -> bool:
    ...
    try:
        ...
        result = subprocess.run(  # noqa: S603
            [psql_path, "-h", host, "-U", user, "-c", "SELECT 1;"], capture_output=True, env=env, timeout=5
        )
    except (subprocess.TimeoutExpired, FileNotFoundError):
        return False
    else:
